...
function lerEHS_() {
  const guardado = lerGrande_('ehs_dados');
  if (guardado) {
  ...
  // Em pedaços, pelo mesmo motivo do RDO: numa chave só o cache morre em
  // silêncio quando a aba cresce.
  guardarGrande_('ehs_dados', JSON.stringify(linhas), CACHE_RDO_SEGUNDOS);
  return linhas;
}
...
/* Mesma ideia do RDO: a lista de clientes e parques muda devagar e não precisa
   ser refeita a cada abertura da tela. */
function acaoEhsFiltrosCache_(p) {
  const guardado = lerGrande_('ehs_filtros');
  if (guardado) {
    try { return JSON.parse(guardado); } catch (e) { /* cache ruim: refaz */ }
  }
  const res = acaoEhsFiltros_(p);
  if (res.ok) guardarGrande_('ehs_filtros', JSON.stringify(res), CACHE_FILTROS_SEG);
  return res;
}
